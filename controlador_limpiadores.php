<?php	
	...
	if (isset($_REQUEST["guardar1"])){
			...
			if(count($errores)>0) {
				...
				Header("Location:EmpleadosPrivado.php#añadirCamareroLimpieza");

			// Si NO se han detectado errores
			} else 
				Header("Location: accion_add_limpiador.php");
	...
	//Lo guardamos en la variable global para poder comprobarlo en EmpleadosPrivado
			...
			$_SESSION["guardarEdit1"] = $_REQUEST["guardarEdit1"];		
			$empleado["dni"] = $_REQUEST["dni"];
			$empleado["email"] = $_REQUEST["email"];
			$empleado["telefono"] = $_REQUEST["telefono"];
			$empleado["turno"] = $_REQUEST["turno"];
			...
			Header("Location: accion_modify_limpiador.php");
			...
			Header("Location: accion_delete_limpiador.php");
...
function validarDatosEmpleado($empleado){
	...
	// Validación de apellidos 
		...
		}else if(is_numeric($empleado["apellidos"])){
			$errores[]="El formato del apellidos no es correcto" . $empleado["apellidos"];
		}
	// Validación de email
		if($empleado["email"]==""){ 
